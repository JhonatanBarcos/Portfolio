...
def main() -> None:
    ...
    # Get the POI's that the plane must visit (in geodetic coordinates)
    POIs = np.array(execution_parameters['POIs'], dtype=np.float32)

    # Discretize POIs from (lat, lon) to (i, j)
    POIs_discretized = discretize_coords(high_level_plan=POIs,
                                         boundaries=boundaries,
                                         map_width=M.width,
                                         map_height=M.height)

    # Verificar que todos los POIs están presentes en el grafo (accesibles)
    for poi in POIs_discretized:
        if tuple(poi) not in G:
            print(f"[ERROR] El POI {tuple(poi)} no está accesible (fuera del grafo por alta detección).")
            print("Prueba con una tolerancia más alta o modifica el escenario.")
            return  # Finaliza el programa para evitar crash en A*

    # Compute the solution with discretized POIs
