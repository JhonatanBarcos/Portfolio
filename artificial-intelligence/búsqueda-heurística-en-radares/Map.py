...
class Map:
    ...
    def compute_detection_map(self) -> np.array:
        ...
        # Creamos las coordenadas geodésicas discretizadas
        ...
        # Paso 1: Φ*(lat, lon) = max Φ_i(lat, lon) si d <= Rmax; 0 si no
        ...
        # Paso 2: Φ_scaled = escalar entre [ε, 1] con fórmula (8)
        min_val = np.min(detection_map)
        max_val = np.max(detection_map)

        # Evitar división entre 0
        if max_val - min_val == 0:
            detection_map_scaled = np.full_like(detection_map, EPSILON)
        else:
            detection_map_scaled = ((detection_map - min_val) / (max_val - min_val)) * (1 - EPSILON) + EPSILON

        return detection_map_scaled